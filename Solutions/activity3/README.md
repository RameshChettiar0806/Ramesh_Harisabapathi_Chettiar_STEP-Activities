...
### `Account.java`
`Account.java` is the enhanced bank account model in the `com.gdb.domain` package. It stores the account number, customer name, age, balance, account type, status, and 4-digit PIN. The constructor validates that the customer is at least 18, the initial balance is not negative, and the PIN is exactly 4 digits. It also includes `validatePin`, `changePin`, `withdraw`, `suspend`, `activate`, and `close` methods to manage security and account state.

### `TestAccount.java`
`TestAccount.java` is the executable test driver in the `com.gdb.tests` package. It creates an account, verifies the initial balance, tests a correct PIN withdrawal, rejects a wrong PIN, blocks withdrawals while the account is suspended, re-activates the account, changes the PIN, and verifies a withdrawal with the new PIN.
...
Run these commands from the `activity3` folder.
...
# Compile both Java files together
...
Separate compilation (copy-paste version):
```powershell
Set-Location "C:\Users\Ramesh\Personal Folders\College\SRM\SEMESTERS\SEMESTER - 5\STEP Classes\19th September\SRMfullstack-main\SRMfullstack-main\Solutions\activity3"
...
```cmd
cd /d "C:\Users\Ramesh\Personal Folders\College\SRM\SEMESTERS\SEMESTER - 5\STEP Classes\19th September\SRMfullstack-main\SRMfullstack-main\Solutions\activity3"
if not exist bin mkdir bin
javac -d bin src\com\gdb\domain\Account.java src\com\gdb\tests\TestAccount.java
java -cp bin com.gdb.tests.TestAccount
```

### Linux & macOS (Terminal / Bash / Zsh)
```bash
cd /path/to/Solutions/activity3
mkdir -p bin
javac -d bin src/com/gdb/domain/Account.java src/com/gdb/tests/TestAccount.java
java -cp bin com.gdb.tests.TestAccount
```
